...
async def add_user(conn: asyncpg.Connection, user_data: Dict) -> Dict:
    """
    Создаёт/обновляет пользователя с ролью 'pending' и approved=False при первом заходе.
    """
    query = """
    ...
    if not result:
        return await get_user(conn, user_data['telegram_id'])
    return _record_to_dict(result)
...
async def get_user(conn: asyncpg.Connection, telegram_id: int) -> Optional[Dict]:
    ...
    SELECT u.*,
           ...
           s.pending_group_id,
           pg.name AS pending_group_name,
           s.group_change_requested_at,
           u.pending_first_name,
           u.pending_last_name,
           u.name_change_requested_at
    FROM users u
...
async def set_student_group(conn: asyncpg.Connection, student_user_id: int, group_id: Optional[int]) -> bool:
    ...
    UPDATE students
       SET group_id = $1,
           ...
           group_change_requested_at = NULL
     WHERE student_id = $2
...
async def request_group_change(conn: asyncpg.Connection, student_user_id: int, new_group_id: int) -> bool:
    query = """
    UPDATE students
       SET pending_group_id = $1,
           group_change_requested_at = CURRENT_TIMESTAMP
     WHERE student_id = $2
     RETURNING student_id;
    """
...
async def get_pending_group_changes(conn: asyncpg.Connection) -> List[Dict]:
 ...
 LEFT JOIN groups cg ON s.group_id = cg.group_id
  ...
  ORDER BY s.group_change_requested_at ASC;
    """
...
async def approve_group_change(conn: asyncpg.Connection, student_user_id: int) -> bool:
    async with conn.transaction():
        pid = await conn.fetchval(
            "SELECT pending_group_id FROM students WHERE student_id = $1 AND pending_group_id IS NOT NULL FOR UPDATE",
            student_user_id,
        )
        if pid is None:
            return False
        status = await conn.execute(
            "UPDATE students SET group_id = $1, pending_group_id = NULL, group_change_requested_at = NULL WHERE student_id = $2;",
            pid,
            student_user_id,
        )
...
async def reject_group_change(conn: asyncpg.Connection, student_user_id: int) -> bool:
    query = """
    UPDATE students
       SET pending_group_id = NULL,
           group_change_requested_at = NULL
     WHERE student_id = $1
       AND pending_group_id IS NOT NULL
 RETURNING student_id;
    """
...
async def request_name_change(conn: asyncpg.Connection, user_id: int, first_name: str, last_name: str) -> bool:
    query = """
    UPDATE users
       SET pending_first_name = $1,
           pending_last_name  = $2,
           name_change_requested_at = CURRENT_TIMESTAMP
     WHERE user_id = $3
 RETURNING user_id;
    """
...
async def get_pending_name_changes(conn: asyncpg.Connection) -> List[Dict]:
    ...
    SELECT user_id,
           ...
           pending_first_name,
           pending_last_name,
           name_change_requested_at
      FROM users
     WHERE pending_first_name IS NOT NULL
  ORDER BY name_change_requested_at ASC;
    """
    return _records_to_list_dicts(await conn.fetch(query))
...
async def approve_name_change(conn: asyncpg.Connection, user_id: int) -> bool:
    async with conn.transaction():
        pending_names = await conn.fetchrow(
            "SELECT pending_first_name, pending_last_name FROM users WHERE user_id = $1 AND pending_first_name IS NOT NULL FOR UPDATE",
            user_id,
        )
        ...
        status = await conn.execute(
            ...
            UPDATE users
             ...
             WHERE user_id = $3;
            """,
            pending_names['pending_first_name'],
            pending_names['pending_last_name'],
            user_id,
        )
        return status == 'UPDATE 1'
...
async def reject_name_change(conn: asyncpg.Connection, user_id: int) -> bool:
    query = """
    UPDATE users
       SET pending_first_name = NULL,
           pending_last_name  = NULL,
           name_change_requested_at = NULL
     WHERE user_id = $1
       AND pending_first_name IS NOT NULL
 RETURNING user_id;
    """
...
async def get_users(conn: asyncpg.Connection, approved: Optional[bool] = None, role: Optional[str] = None,
 ...
 LEFT JOIN groups pg ON s.pending_group_id = pg.group_id
    ...
    if approved is not None:
        conditions.append(f"u.approved = ${idx}"); params.append(approved); idx += 1
    if role is not None:
        conditions.append(f"u.role = ${idx}"); params.append(role); idx += 1
    if group_id is not None:
        conditions.append(f"s.group_id = ${idx}"); params.append(group_id); idx += 1
    if conditions:
...
async def set_user_role(conn: asyncpg.Connection, user_id: int, role: str) -> Optional[Dict]:
    ...
    async with conn.transaction():
        res = await conn.fetchrow(
            "UPDATE users SET role = $1 WHERE user_id = $2 RETURNING user_id, role;",
            role, user_id
        )
        if not res:
            return None
...
async def get_assignment(conn: asyncpg.Connection, assignment_id: int) -> Optional[Dict]:
    result = await conn.fetchrow("SELECT * FROM assignments WHERE assignment_id = $1", assignment_id)
    return _record_to_dict(result)


async def get_assignments_for_group(conn: asyncpg.Connection, group_id: int) -> List[Dict]:
    """
    В разных схемах колонка даты может называться creation_date или created_at.
    Для совместимости сортируем по COALESCE(created_at, creation_date, assignment_id).
    """
    query = """
    SELECT assignment_id, title, due_date, accepting_submissions,
           COALESCE(created_at, creation_date, NOW()) AS created_ord
      FROM assignments
     WHERE group_id = $1
  ORDER BY created_ord DESC, assignment_id DESC
    """
...
async def toggle_accept_submissions(conn: asyncpg.Connection, assignment_id: int, accept: bool) -> Optional[Dict]:
    result = await conn.fetchrow(
        "UPDATE assignments SET accepting_submissions = $1 WHERE assignment_id = $2 RETURNING assignment_id, accepting_submissions;",
...
async def add_submission(conn: asyncpg.Connection, assignment_id: int, student_user_id: int, file_id: str) -> Dict:
    async with conn.transaction():
        assignment = await conn.fetchrow(
            "SELECT due_date FROM assignments WHERE assignment_id = $1 FOR UPDATE",
            assignment_id
        )
        ...
        ON CONFLICT (assignment_id, student_id) DO UPDATE SET
            ...
            submitted = TRUE,
            grade = NULL, score1 = NULL, score2 = NULL,
            graded_by = NULL, grade_date = NULL, teacher_comment = NULL
...
async def update_submission_grade(
 ...
 RETURNING submission_id, grade, score1, score2, grade_date;
    """
...
async def get_grades_for_student(conn: asyncpg.Connection, student_user_id: int) -> List[Dict]:
  ...
  ORDER BY COALESCE(a.created_at, a.creation_date, NOW()) DESC,
...
async def add_question(
    ...
    """
    Создаёт вопрос от студента.
    Возвращает: {question_id: int}
    """
    row = await conn.fetchrow(
        """
        INSERT INTO questions (student_id, group_id, question_text, asked_at)
        VALUES ($1, $2, $3, NOW())
        RETURNING question_id
        """,
        student_id, group_id, question_text
    )
...
async def get_questions(
    ...
    """
    Список вопросов с расширенной информацией:
    - имя/фамилия студента, группа
    - ответ/кто ответил/когда
    Фильтр answered:
      None  -> все;
      True  -> только с ответом;
      False -> только без ответа.
    """
    base = """
        SELECT
            q.question_id, q.student_id, q.group_id, q.question_text, q.asked_at,
    ...
    params: List[Any] = []
    if answered is True:
        where.append("q.answer_text IS NOT NULL")
    elif answered is False:
        where.append("q.answer_text IS NULL")
    if where:
        base += " WHERE " + " AND ".join(where)
    base += " ORDER BY q.asked_at DESC"
    rows = await conn.fetch(base, *params)
    return _records_to_list_dicts(rows)
...
async def add_answer(
    ...
    """
    Записывает ответ преподавателя на вопрос (если ещё не отвечен).
    Возвращает словарь с полями вопроса + student_telegram_id для нотификации.
    """
    async with conn.transaction():
        row = await conn.fetchrow(
            """
            ...
            WHERE question_id=$3 AND answer_text IS NULL
            RETURNING question_id, student_id, question_text, answer_text, answered_at
            """,
            answer_text, teacher_user_id, question_id
        )
        if not row:
            return None
        ...
        out["student_telegram_id"] = student["telegram_id"] if student else None
        return out
