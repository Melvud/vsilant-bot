-- =========================================================
-- PostgreSQL schema for Telegram LMS (админ-панель + бот)
-- Включает: группы, пользователи, студенты/преподаватели,
-- задания, сдачи, посещаемость, Q&A, материалы (NEW).
-- =========================================================

-- безопасные расширения (опционально)
...
CREATE TABLE IF NOT EXISTS users (
    ...
    role           VARCHAR(50)  NOT NULL DEFAULT 'pending'
                   CHECK (role IN ('student', 'teacher', 'pending')),
    approved       BOOLEAN      NOT NULL DEFAULT FALSE,
    registration_date TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,

    -- запросы на смену имени (для одобрения в админке)
...
CREATE TABLE IF NOT EXISTS students (
    ...
    -- запрос на смену группы (для одобрения в админке)
...
CREATE TABLE IF NOT EXISTS assignments (
    ...
    description    TEXT,
    file_id        VARCHAR(255),
    file_type      VARCHAR(50), -- 'photo' | 'document' | 'video' | др.
...
------------------------------------------------------------
-- Материалы (новое: лекции/объявления/видео/ссылки/и т.п.)
------------------------------------------------------------
CREATE TABLE IF NOT EXISTS materials (
    material_id  SERIAL PRIMARY KEY,
    group_id     INT NOT NULL REFERENCES groups(group_id) ON DELETE CASCADE,
    category     VARCHAR(50) NOT NULL,  -- lectures | announcements | figures | tasks | video | links | library | activities | other
    title        VARCHAR(255) NOT NULL,
    description  TEXT,
    created_by   BIGINT REFERENCES users(user_id) ON DELETE SET NULL,
    created_at   TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
    -- CHECK по category можно добавить при желании,
    -- но оставляем свободным для расширения.
);
...
CREATE TABLE IF NOT EXISTS material_files (
    ...
    file_type   VARCHAR(50)  NOT NULL,          -- photo | document | video ...
    name        VARCHAR(255),                   -- исходное имя/подпись
    mime        VARCHAR(127),                   -- mime-type, опционально
    created_at  TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE INDEX IF NOT EXISTS idx_students_pending_gid ON students(pending_group_id);
...
CREATE INDEX IF NOT EXISTS idx_attendance_grp_date  ON attendance(group_id, attendance_date);
CREATE INDEX IF NOT EXISTS idx_attendance_stu_date  ON attendance(student_id, attendance_date);
...
CREATE INDEX IF NOT EXISTS idx_mfiles_material_id   ON material_files(material_id);
CREATE INDEX IF NOT EXISTS idx_mlinks_material_id   ON material_links(material_id);
